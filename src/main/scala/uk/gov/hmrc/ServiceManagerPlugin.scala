...
class ServiceManagerConfiguration(externalServices: List[ExternalService]) {
  ...
  val cloneConfiguration = s"rm -rf $serviceManagerWorkspaceDirectoryPath/service-manager-config" #&& s"git clone git@github.tools.tax.service.gov.uk:HMRC/service-manager-config.git $serviceManagerWorkspaceDirectoryPath/service-manager-config"
