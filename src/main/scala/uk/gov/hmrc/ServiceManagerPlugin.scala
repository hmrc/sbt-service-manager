...
class ServiceManagerConfiguration(externalServices: List[ExternalService]) {
  ...
  val cloneConfiguration = s"rm -rf $serviceManagerWorkspaceDirectoryPath/service-manager-config" #&& s"git clone git@githubcom:HMRC/service-manager-config.git $serviceManagerWorkspaceDirectoryPath/service-manager-config"
